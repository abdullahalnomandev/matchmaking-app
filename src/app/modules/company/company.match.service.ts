...
const getMatchableCompanies = async (currentUser: {
  ...
  try {
    ...
    }).populate('owner', 'name email image psychological_scores personality_scores mbti_type ranking_score verified role');
    ...
    if (query?.rank_level) {
      ...
      const filteredCompanies = allCompanies.filter(company => {
        ...
        // Exclude companies owned by support_partner users
        if (ownerData?.role === USER_ROLES.SUPPORT_PARTNER) {
          return false;
        }
    ...
    // Filter out companies owned by support_partner users
    const filteredMatches = potentialMatches.filter(company => {
      const ownerData = company.owner as any;
      return ownerData?.role !== USER_ROLES.SUPPORT_PARTNER;
    });
    
    // Debug: Log each potential match
    filteredMatches.forEach((company, index) => {
    ...
    const matchResults = filteredMatches.map(company => 
      calculateMatchScore(currentUserForMatching, company)
    );
...
const getMatchCountCompanies = async (currentUser: {
  ...
  try {
    ...
    const potentialMatches = await Company.find({
      owner: { $ne: currentUser.id }
    }).populate('owner', 'name email image psychological_scores personality_scores mbti_type ranking_score verified role').lean();
    
    // Filter out companies owned by support_partner users
    const filteredByRole = potentialMatches.filter(company => {
      const ownerData = company.owner as any;
      return ownerData?.role !== USER_ROLES.SUPPORT_PARTNER;
    });
    
    // Filter for verified users only - TEMPORARILY COMMENTED OUT FOR DEBUGGING
    // const verifiedMatches = filteredByRole.filter(company => 
    //   (company.owner as any)?.verified === true
    // );
    const verifiedMatches = filteredByRole; // Use all matches for debugging
